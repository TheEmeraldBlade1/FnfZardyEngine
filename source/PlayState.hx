...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if (dad.curCharacter == 'zardy')
			healthBar.createFilledBar(0xFFBA7B42, 0xFF303030);
		else if (dad.curCharacter == 'dad')
			healthBar.createFilledBar(0xFFAF66CE, 0xFF303030);
		else if (dad.curCharacter == 'gf')
			healthBar.createFilledBar(0xFFA5004D, 0xFF303030);
		else if (dad.curCharacter == 'bf')
			healthBar.createFilledBar(0xFF31B0D1, 0xFF303030);
		else
			healthBar.createFilledBar(0xFF303030, 0xFF303030);
		// healthBar
		add(healthBar);

		// Add Kade Engine watermark
		var kadeEngineWatermark = new FlxText(4,healthBarBG.y + 50,0,SONG.song + " " + (storyDifficulty == 2 ? "Hard" : storyDifficulty == 1 ? "Normal" : "Easy") + " - ZE " + MainMenuState.kadeEngineVer, 16);
		kadeEngineWatermark.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		kadeEngineWatermark.scrollFactor.set();
		if (FlxG.save.data.kadeshow)
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			switch (curStage)
			{
				...
				default:
					if (dad.curCharacter == 'zardy')
					{
						if (player == 0)
						{
							babyArrow.frames = Paths.getSparrowAtlas('zardynotes');
						}
						else
						{
			...
			if (FlxG.save.data.flipped)
			{
				babyArrow.flipY = true;
			}

			if (FlxG.save.data.flipped2)
			{
	...
	function noteMiss(direction:Int = 1):Void
	{
		...
		function gnoteMiss(direction:Int = 1):Void
			{
				if (!boyfriend.stunned)
				{
					health -= 0.04;
